...
(defun convert-dot-dash-namespace (clojure-ns)
  (interactive)
  (let* ((no-dash-ns (replace-regexp-in-string "-" "_" clojure-ns)))
    (replace-regexp-in-string (regexp-quote ".") "/" no-dash-ns)))
(defun get-relative-root (project-root ns-no-dot-dash filename file-ext)
  (let* ((drop-pr (string-remove-prefix project-root filename))
         (suffix (concat "/" ns-no-dot-dash "." file-ext)))
    (string-remove-suffix suffix drop-pr)))
;; (defun get-src-or-test (src-dir extn)
;;   (interactive)
;;   (concat src-or-test (if (string-match-p (regexp-quote "/") src-dir)
;;                           (let* ((after-src (string-remove-prefix "src/" src-dir)))
;;                             (cond ((string= "cljc" after-src) "/clj")
;;                                   ((string= "clj" after-src) "/clj")
;;                                   ((string= "cljs" after-src) "/cljs"))))))
(defun get-src-or-test (relative-dir extn)
  (interactive)
  (if (string-prefix-p "src" relative-dir)
      (cond ((string= extn "cljc") "test/clj")
            (t (concat "test" extn))))
  (if (string-prefix-p "test" relative-dir)
      (cond ((string= extn "clj" )))
      )
  (concat src-or-test (if (string-match-p (regexp-quote "/") src-dir)
                          (let* ((after-src (string-remove-prefix "src/" src-dir)))
                            (cond ((string= "cljc" after-src) "/clj")
                                  ((string= "clj" after-src) "/clj")
                                  ((string= "cljs" after-src) "/cljs"))))))
(defun get-test-file (buffer-file-name))
(defun open-test-file ()
  "open the corresponding test file to this implementation file"
  (interactive)
  (let* ((file-extension (file-name-extension (buffer-file-name)))
         (relative-root (get-relative-root (projectile-project-root)
                                           (convert-dot-dash-namespace (clojure-find-ns))
                                     (buffer-file-name)
                                     (file-name-extension (buffer-file-name))))
         (test-dir (get-src-or-test-dir relative-root "test"))
         (test-file
          (concat (projectile-project-root)
                  test-dir "/"
                  (convert-dot-dash-namespace (clojure-find-ns))
                  "_test." file-extension)))
